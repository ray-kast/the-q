...
pub struct MentionableTimestamp {
...
impl MentionableTimestamp {
    ...
    #[must_use]
    pub fn from_seconds_since_unix_epoch(
        seconds_since_unix_epoch: i64,
    ) -> Option<MentionableTimestamp> {
        // TODO: should be Result like TryFrom
        match seconds_since_unix_epoch {
            seconds if seconds < Self::MIN_SECONDS => None,
            seconds if seconds > Self::MAX_SECONDS => None,
            _ => Some(MentionableTimestamp {
    ...
    #[must_use]
    pub fn mention(&self, style: TimestampStyle) -> Mention {
        Mention::Timestamp(self.clone(), style)
    }
}
...
impl Display for Mention {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Mention::Timestamp(timestamp, style) => f.write_fmt(format_args!(
                "<t:{}:{}>",
