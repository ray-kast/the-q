...
mod storage;
use storage::{SleeperStorage, AccusationTransaction, Timestamp, RecordOutcomeError};
...
impl<S: SleeperStorage> SleeperCommand<S> {
    ...
    async fn accuse<'a>(
        ...
        /* restrict the lifetime of voice_states */ {
            let voice_states = ctx.cache.guild_field(gid, |guild| guild.voice_states.clone()).context("Missing guild")?;

            let Some(channel_id) = voice_states.get(&user.id).and_then(|c| c.channel_id) else {
                return Err(
                    responder
                        .create_message(Message::plain("Please connect to a voice channel first.").ephemeral(true)).await
            ...
            if voice_states.get(&target_user.id).and_then(|c| c.channel_id) != Some(channel_id) {
                return Err(
                    responder
                        .create_message(Message::rich(|builder| builder.user(target_user).push(" is not in your voice channel!")).ephemeral(true)).await
                        .context("Error sending error message")?
        ...
        if target_user.bot {
            return Err(
                responder
                    .create_message(Message::plain("Bots never sleep, silly!").ephemeral(true)).await
                    .context("Error sending bot user error")?
                    .into_err("Target user was a bot")
            )
        }

        let transaction = self.storage.begin_accuse(gid, target_user.id, Timestamp::of_interaction(visitor.id())).await.context("Error accusing user")?;

        let deadline = transaction.accusation_time().checked_add(GRACE_PERIOD).expect("deadline is not in the distant past or future");
        ...
        let responder = match transaction.last_accusation() {
            ...
            _ => {
                responder
                    .create_message(Message::rich(|builder| builder
                        .push_line(
                            format_args!("{user} has accused {target_user} of being The Sleeper!", 
                            user = user.mention(), target_user = target_user.mention()
                        ))
                        .push(
                            format_args!("{target_user} will be The Sleeper {deadline} unless they run ", 
                            target_user = target_user.mention(), 
                            deadline = deadline.mentionable().expect("deadline is not in the distant past or future").mention(TimestampStyle::RelativeTime) // TODO: handle this
                        ))
                        .push_mono_safe(format!("/{} {}", self.name, Self::AWAKE_NAME)).push_line(" first!") // TODO: use a command mention here
                    ).ping_users(vec![target_user.id]))
                    .await.context("Error sending accusation notification")?
            }
        };
        ...
        let expire = async {
            ...
            match self.storage.record_outcome(gid, user.id, deadline, Outcome::TheSleeper).await {
                ...
                Err(RecordOutcomeError::NotAccused) => Err(anyhow!("Accusation was missing at time of expiry")),
        ...
        match resolve.map(Ok).or(expire).await? {
            Outcome::TheSleeper => {
                let build_msg = || Message::rich(|builder| builder.push_line(format_args!(
                    "{target_user} is The Sleeper!", target_user = target_user.mention()
                ))).ping_users(vec![target_user.id]);
                ...
                let voice_state = ctx.cache.guild_field(gid, |guild| guild.voice_states.get(&target_user.id).cloned()).context("Missing guild")?;
                if let Some(VoiceState { channel_id: Some(_), .. }) = voice_state {
                    match self.sleeper_action(ctx, gid)? {
                        SleeperAction::Disconnect => gid.disconnect_member(&ctx.http, target_user).await.context("Error disconnecting user"),
                        SleeperAction::MoveToChannel(afk_channel_id) => gid.move_member(&ctx.http, target_user, afk_channel_id).await.context("Error moving user to AFK channel"),
                    }.map_err(|e| error!(?e)).ok(); // TODO: use inspect_err when available
                }
            },  
            Outcome::NotTheSleeper => {
                let build_msg = || Message::rich(|builder| builder.push_line(format_args!(
                    "{target_user} was not The Sleeper! Sorry, {user}!", target_user = target_user.mention(), user = user.mention()
    ...
    async fn awake<'a>(
        ...
        let responder = match result {
            Ok(()) => responder
                .create_message(Message::plain("You have successfully refuted the accusation of being The Sleeper.").ephemeral(true)).await 
                .context("Error sending refutation acknowledgement")?,
            Err(RecordOutcomeError::AlreadyResolved(Outcome::TheSleeper)) => return Err(responder
                .create_message(Message::plain("Sorry, you were too late to clear your name this time.").ephemeral(true)).await 
                .context("Error sending too-late error")?
                .into_err("User was too late to defend themself")),
            Err(RecordOutcomeError::AlreadyResolved(Outcome::NotTheSleeper)) => return Err(responder
                .create_message(Message::plain("You have already cleared your name this time.").ephemeral(true)).await 
                .context("Error sending already-resolved error")?
                .into_err("User had already indicated they were awake")),
            Err(RecordOutcomeError::NotAccused) => return Err(responder
                .create_message(Message::plain("You are not currently accused of being The Sleeper.").ephemeral(true)).await 
...
#[async_trait]
impl<S: SleeperStorage + Debug + Sync + Send + 'static> CommandHandler<Schema> for SleeperCommand<S> {
